...
# people = [{"name":name, "status":status, "age":age, "email":email} for name, status, age, email in people]
people_age = [x for x in people if x[2]>20]
people_age.sort(key=lambda x: x[2])
